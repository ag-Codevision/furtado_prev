...
async function runMigration() {
    ...
    try {
        ...
        const sqlPath = path.join(__dirname, '..', 'supabase', 'migrations', '20260114_extend_profiles_table.sql');
        const sql = fs.readFileSync(sqlPath, 'utf8');

        console.log('Executando script SQL de extensão...');
        await client.query(sql);

        console.log('Migração de extensão concluída com sucesso!');
    } catch (err) {
